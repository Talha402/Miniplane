...
async def kang(event):
    ...
    packtitle = (f"@{user.username or user.first_name}'s MiniPlane Pack "
    ...
    if not muted: # Tell the user just in case, this may rarely happen
        await event.edit(
            "`MiniPlane couldn't mute the Stickers bot, beware of notification spam.`")

    if new_pack:
        await event.edit("`This MiniPlane Sticker Pack doesn't exist! Creating a new pack...`")
