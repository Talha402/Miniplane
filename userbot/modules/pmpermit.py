...
# ========================= CONSTANTS ============================
UNAPPROVED_MSG = (
    "Hello stranger! It seems you are PMing my master\n\n"
    "My master hasn't allowed you to PM yet."
    "Please wait untill he sees your PM\n\n"
    "Note: Do not spam. If you do I will report and block you.")
# =================================================================
...
async def approvepm(apprvpm):
    ...
    if await approve(apprvpm.chat_id) is False:
        return await apprvpm.edit("`User was already allowed!`")
    else:
        ...
        await apprvpm.edit(f"[{name0}](tg://user?id={uid}) `allowed to PM!`")
...
async def blockpm(block):
    ...
    if await block_pm(block.chat_id) is False:
        return await block.edit("`First allow, before blocking`")
    else:
        await block.edit("`You are gonna be blocked from PMing my Master!`")
...
CMD_HELP.update({
    "pmpermit": [
        "PMPermit",
        " - `.approve`: Allow the mentioned/replied person to PM.\n"
        " - `.block`: Blocks the person from PMing you.\n"
        " - `.unblock`: Unblocks the person, so they can PM you again.\n"
        " - `.notifoff`: Stop any notifications coming from unallowed PMs.\n"
        " - `.notifon`: Allow notifications coming from unallowed PMs.\n"
    ]
})
